...
5. Packages are the only piece I don't have a good strategy for yet. Here are the strategies I have tried:
    ...
    - Using `cx_freeze` inside a `virtualenv` to compile your script into an executable which can be called from outside the `virtualenv`. This might also benefit from a shell "launcher" that checks to see if the binary is out-of-date with the source.
...
- [Beyond Bash by Li Haoyi](https://docs.google.com/presentation/d/11vZzXCfAA0aOFAuHA0nAvAzALGFGCH-dqHxx6XMgbk8) is an excellent exposition on why Bash needs to change, but maybe Scala is a better replacement than Python. The author presents Ammonite Ops, a Scala-based library with the goal of Ammonite is to make Scala scripts no more than 2x more characters than the equivalent Bash script. How can anyone be confident that a deployment/workflow _works_? In Bash, the best you can do is "it seems to work," whereas Scala can make much stronger guarantees through the type-checking.

- [The Unix Chainsaw by Gary Bernhardt](https://www.youtube.com/watch?v=sCZJblyT_XM) espouses a seemingly opposite viewpoint, but I think it is mostly consistent with my view. Gary Bernhardt argues (and I agree) that shell is be extremely useful and powerful. I would caveat that shell is often less maintainable than real programming languages. Gary Berhardt points out that not all things need to be maintainable (they can be "half-assed"). Many people use shell one-liners for things they only have to do once, and I think that is good. My problem is when shell scripts start taking the place of a real program. This goes back to Li Haoyi's point, "how can you be sure that a deployment is going to still work with new inputs if it is written in shell?" Gary Bernhardt actually touches on a fundamental limitations: "You can compose streams through pipes, but you can't build a new named stream, because it doesn't really have that abstraction yet" (transcribed liberally).
