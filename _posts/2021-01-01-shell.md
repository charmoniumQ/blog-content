...
Quoting does not even solve the problem completely. Suppose `var='a "b c"'`. Then `"$var"` expands to one argument (`a "b c"`), while `"$var"` expands to three (`a`, `"b`, and `c"`). As far as I know, there is no way of making `var` expand to two arguments (`a` and `b c`) short of writing your own lexer in shell or calling `eval` (according to the all-knowing [Stack Overflow](https://superuser.com/questions/1066455/how-to-split-a-string-with-quotes-like-command-arguments-in-bash)). Bash tries to fix this with arrays, (`var=(a "b c")` and `"${var[@]}"`), but critically there is no easy way to pass multiple bash arrays to another script or even another function in the same script.
...
4. If you _have_ to talk to some other program by a command-line interface,
    ...
    if __name__ == '__main__':
        ...
        def main(...):
            format = ("text"
                      if format == "auto" and sys.stdout.isatty()
                      else "json")
...
Most systems engineers I know worship the UNIX shell and its philosophy, so I have braced myself for a reaction (looking at you, HN). 98% of the time, I agree with them, and the other 2% worked really well for three decades. I am genuinely interested in opposing opinions, so drop a comment explaining why shell scripts are useful.
